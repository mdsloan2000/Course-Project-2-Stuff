...
# Build the Data Set
        ...
        NEI <- subset(NEI, fips == "24510" | fips == "06037")  #Subsets the Baltimore Data AND Los Angelas County
        NEI <- NEI[NEI$SCC %in% SCC_codes,] #subsets the data based on the Onroad List.
        NEI <- ddply(NEI, .(year, fips), summarize, Emissions = (sum(Emissions)))

# plot commands go here.
        p <- qplot(as.character(year), Emissions, data=NEI, colour=fips) 
        p <- p + ylab("Fuel Emissions From Motor Vehices (Baltimore City vs LA County") + xlab ("Emissions per Year")
        print(p)
